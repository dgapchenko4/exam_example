...
### Шаг 2.1. Переименование класса Brand в products/models.py

1. Откройте `products/models.py`
2. Найдите класс `Brand`:
...
Django может спросить, переименовывать ли модель Brand в Brand. Выберите вариант **«y»** (переименование), если он предлагается. Если Django создаёт новую таблицу и удаляет старую — данные производителей будут потеряны.
...
2. Замените импорт:
   ...
   ```
3. Найдите все вхождения `Brand` и замените на `Brand`
